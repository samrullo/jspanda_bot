...
class FamilySpendingController:
    ...
    def family_spending_month(self, adate):
        ...
        spending_df = pd.DataFrame(columns=['id', 'date', 'name', 'amount'], data=all_records)
    ...
    def family_spending_edit(self, id):
        ...
        if form.validate_on_submit():
            ...
            flash(f"Updated {id} with {date}, {name}, {amount}", "success")
            return self.family_spending_month(date)
        record = self.model.get_item(id)
        form.id.data = id
        form.date.data = record[1]
        form.name.data = record[2]
        form.amount.data = record[3]
        return render_template("family_spending/family_spending_edit.html", form=form)
